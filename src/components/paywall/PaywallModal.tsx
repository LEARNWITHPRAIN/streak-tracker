...
export const PaywallModal: React.FC<PaywallModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      {/* Modal */}
      <div className="relative w-full sm:max-w-lg max-h-[95vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-card/95 backdrop-blur-2xl border border-primary/30 shadow-2xl shadow-primary/20 animate-slide-up">
        {/* Orange gradient top bar */}
        <div className="h-1.5 w-full bg-gradient-to-r from-orange-500 via-amber-500 to-orange-600 rounded-t-3xl sm:rounded-t-3xl" />
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1 rounded-full bg-gradient-to-r from-orange-500 to-amber-600 text-primary-foreground shadow-lg shadow-primary/25">
          ...
          <div className="space-y-3">
            ...
            {BENEFITS.map((b) => (
              <div
                key={b.title}
                className="flex items-start gap-3 p-3.5 rounded-2xl bg-muted/40 border border-border/70"
              >
                <div className="p-2 rounded-xl flex-shrink-0 bg-primary/15 text-primary">
          ...
          {/* Free vs Pro table */}
          <div className="rounded-2xl border border-border/80 overflow-hidden bg-background/50">
            <div className="grid grid-cols-3 px-4 py-2.5 bg-muted/40 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
              <span>Feature</span>
              <span className="text-center">Free</span>
              <span className="text-center text-primary font-extrabold">Pro</span>
            </div>
            {FREE_VS_PRO.map((row, i) => (
              <div
                key={row.label}
                className={`grid grid-cols-3 px-4 py-2.5 text-xs border-t border-border/60 ${
                  i % 2 === 0 ? '' : 'bg-muted/20'
                }`}
              >
                ...
                <span className="text-center">
                  <Check className="w-3.5 h-3.5 text-primary font-bold mx-auto" />
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="text-center">
              ...
              <p className="text-[10px] text-primary/80 mt-0.5 font-semibold">
            ...
            <Button
              ...
              disabled={loading}
              className="w-full h-12 text-base font-bold rounded-xl bg-gradient-to-r from-orange-500 via-amber-500 to-orange-600 hover:from-orange-600 hover:to-amber-600 text-primary-foreground shadow-xl shadow-primary/25 transition-all duration-200 hover:shadow-primary/40 hover:scale-[1.02]"
            >
